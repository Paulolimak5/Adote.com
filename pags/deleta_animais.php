...
<form method="POST" id="editar_animal" class="col-md-6 offset-md-3">
    <p class="col-md-12 offset-md-4" >Dados do animal<p>
    <label>Nome</label>
    ...
    <label>Imagem</label>
    <input type="text" name="img" value="<?php echo $dados['img'] ?>" class="form-control"><br>

    <label>Texto</label>
    <input type="text" name="txt" value="<?php echo $dados['txt'] ?>" class="form-control"><br>


    <input type="submit" value="Deletar animal" class="btn btn-outline-danger btn-lg btn-block">
    <input type="hidden" name="env" value="deleta_animal">
...
<?php
if(isset($_POST['env'])){
    if($_POST['env'] && $_POST['env'] == "deleta_animal"){
        if($_POST['nome'] && $_POST['idade'] && $_POST['peso'] && $_POST['tipo'] && $_POST['img'] && $_POST['txt']){
            ...
            $query = $con->prepare("DELETE FROM animal WHERE id = ?");
            $query->bind_param("s", $id);
            $query->execute();

            if($query->affected_rows > 0){
                echo "<br> <div class='alert alert-success col-md-6 offset-md-3'>Animal deletado com sucesso!</div>";
            }else{
                echo "<br> <div class='alert alert-danger col-md-6 offset-md-3'>Erro ao deletar animal!</div>";
            }
