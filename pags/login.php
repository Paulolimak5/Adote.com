            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarResponsive">
                    <ul class="navbar-nav ml-auto">
                        ...
                        <li class="nav-item mx-0 mx-lg-1"><a class="nav-link py-3 px-0 px-lg-3 rounded js-scroll-trigger" href="cadastro">Cadastre-se</a></li>
                    </ul>
                </div>
            </div>
        </nav>
...
<div id="login" class="col-sm-6 offset-md-3">
    <div class="title01">Entrar</div>
    <div class="content">
        <form method="POST">
            <input type="text" name="usuario" placeholder="Usuario..." class="form-control">
            <input type="password" name="senha" placeholder="Senha..." class="form-control">

            <input type="submit" name="Entrar" class="btn btn-primary btn-lg btn-block">
            <input type="hidden" name="env" value="login">
        </form>
        <div><a href="recupera">Recuperar senha</a></div>
        ...
        if(isset($_POST['env'])){
            if($_POST['env'] && $_POST['env'] == "login"){
                if($_POST['usuario'] && $_POST['senha']){
                    ...
                    if($total > 0){
                        $_SESSION['usuarioLogin'] = $dados['usuario'];
                        $_SESSION['usuarioNome'] = $dados['nome'];
                        $_SESSION['usuarioId'] = $dados['id'];

                        if($dados['nivel'] == '1'){
                            header('Location: adm/');
                            echo "<div class='alert alert-success col-sm-6 offset-md-3'>Logado com sucesso!</div>";
                        }else{
                        header('Location: inicio/');
                        echo "<div class='alert alert-success col-sm-6 offset-md-3'>Logado com sucesso!</div>";
                    }
                    }else{
                        echo "<div class='alert alert-danger col-sm-6 offset-md-3'>Usuário ou senha invalida!</div>";
                    }
                }else{
                    echo "<div class='alert alert-warning col-sm-6 offset-md-3'> Preencha todos os campos</div>";
                }
            }
        }
        ?>
